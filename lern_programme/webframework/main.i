...
# 8 "defined.h" 2
# 1 "funcs.h" 1
extern int field[4][1024][1024], setpoint, player[4][1024][1024];
extern int a, b, c, d, e, f, g, h, i, j, k, l, m, n, o, p, q, r, s, t, u, v, w, x, y, z, word, string, counter_putintable, column, column_, row_pit, row_, lastrowx, mode, limiter, limiter_, limiter_column , check, lastrow;
...
int print_field(int field[4][1024][1024], int mode, int limiter_row, int limiter_column, int limiter_);
int init(void);
...
char *parse_content(char *content, char *gap_allocate, char *gap_gap, int switcher){
    ...
    char *string_a=(char*) malloc(strlen(content) * sizeof(char));
    strcpy(string_a, content);
    ...
    for(parse_counter=0; parse_counter<20; parse_counter++){
        ...
        if(switcher==0){
        sprintf(_GET[parse_counter][0], "%s",ausgabe_string);
        }else{
            sprintf(_POST[parse_counter][0], "%s",ausgabe_string);
        }
        ...
        if(switcher==0){
            ...
            sprintf(_GET[parse_counter][1], "%s",ausgabe_content);
        }else{

            sprintf(_POST[parse_counter][1], "%s",ausgabe_content);
        }
# 98 "funcs.h"
        if(string_c!=((void *)0)){
        free(string_c);
...
char puffer_[1024];
int field[4][1024][1024], setpoint, player[4][1024][1024];
int a, b, c, d, e, f, g, h, i, j, k, l, m, n, o, p, q, r, s, t, u, v, w, x, y, z, word, string, counter_putintable, column, column_, row_pit, row_, lastrowx, mode, limiter, limiter_, limiter_column , check, zahl, pos, false_var_error, false_var_error_=0, ggv_c;
char character[1], character__[sizeof(1024)];
...
int main()
{
    ...
    parse_content("method=normal&action=post", "=", "&", 0);
                ...
                if(strcmp(_GET[1][1],"post")==0){
                    ...
                    if(content_length==((void *)0)){
                        ...
                        if(size<=0){
                            ...
                            if(content_post==((void *)0)){
                                free(content_post);
                                goto exit;
                            }
                            if(fgets(content_post, size+1, stdin)==((void *)0)){
                                free(content_post);
                                goto exit;
                            }

                            parse_content(content_post, "=", "&", 1);
